    ...
    return (
        <div className="space-y-8 mt-6 pt-6 border-t">
            {detalhes_prato?.alimentos?.length > 0 && (
                <div className="mb-6">
                    <h3 className="font-semibold text-md text-green-800 mb-2 text-left">Itens da Análise:</h3>
                    <div className="overflow-hidden rounded-lg border border-gray-200 shadow-md">
                        <table className="min-w-full bg-white">
                            <thead className="bg-gray-50">
                                <tr>
...
export default function Home() {
    ...
    const fetchDetailedAnalysis = async () => {
        if (!scanResult?.resultado?.alimentos_extraidos || scanResult.resultado.alimentos_extraidos.length === 0) {
            setAnalysisError("Não há alimentos confirmados para analisar.");
            return;
        }
        ...
        const alimentosParaAnalisar = scanResult.resultado.alimentos_extraidos.map(alimento => ({
            nome: alimento.nome,
            quantidade_gramas: alimento.quantidade_estimada_g
        }));

        const payload = { alimentos: alimentosParaAnalisar };

        try {
            // ✅ CORREÇÃO: Apontando para o novo endpoint que recebe a lista
            const response = await api.post<AnaliseCompletaResponse>('/refeicoes/analisar-lista-detalhada', payload); 
            ...
            if (error instanceof AxiosError) {
                const detail = error.response?.data?.detail;
                // Tenta extrair a mensagem de erro da validação do FastAPI se existir
                let specificError = defaultErrorMessage;
                if (detail && Array.isArray(detail) && detail[0]?.msg) {
                    specificError = detail[0].msg; // Pega a primeira mensagem de erro de validação
                } else if (typeof detail === 'string') {
                    specificError = detail;
                } else if (error.message) {
                    specificError = error.message;
                }
                setAnalysisError(specificError);
            } else if (error instanceof Error) {
